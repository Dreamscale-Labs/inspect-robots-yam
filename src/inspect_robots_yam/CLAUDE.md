...
| `poses.py` | Pure versioned JSON store for named 14-D start poses, including the filename-safe name rule, validation, listing, deletion, and metadata-preserving rename. |
| `config.py` | `YamConfig` / `ActServerConfig` (with the `MolmoActConfig` alias; frozen, `from_kwargs` for CLI scalars), including the opt-in `report_joint_eff` runtime-state flag, plus joint and EEF action/observation-space builders. |
| `operator.py` | `OperatorIO` (injectable stdin/stdout) for the never-connected readiness fallback; `default_poll_end` (real TTY poll, `# pragma: no cover`); `stdin_interactive`, the TTY probe behind `auto_start`'s pre-motion fail-fast. Verdicts + grader notes are the framework prompt's job. |
...
| `embodiment.py` | `YAMEmbodiment`: i2rt driver with joint and EEF control. Clamp backstop, optional strict abort-only absolute-policy validation with an explicit recorded gripper-endpoint projection for raw DreamZero-YAM overshoot, no-motion preparation observation, per-camera epoch capture times with separate monotonic freshness, optional delta→abs, lazy kinematics, opt-in undeclared `joint_eff` runtime state, default-on collision-guardrail contribution with explicit skip warnings, gripper de-norm, `SELF_PACED` pacing, operator-keypress episode end (`operator_end`), joint-space homing/parking, and `observe_parked()` final grader views. Its `connect_operator_session()` hook yields terminal ownership and routes status, notices, and readiness gates through the framework session; `defer_operator_end()` remains the older stdin-only hook. Hardware seams are injected/pragma'd. |
...
| `pose_cli.py` | Zero-gravity named-pose capture, safe goto/park workflows, and store-only pose management with injected driver, console, clock, sleep, and hostname seams. |
